...
export function conteudoPublico(d, publicar = false) {
  ...
  if (d.gradeCardapio != null) {
    if (p.tipo !== "cardapio")
      throw new Error("A grade mensal só pode ser usada em cardápios.");
    p.gradeCardapio = validarCardapio(d.gradeCardapio, {
      mes: p.numero,
      publicar,
    });
    p.texto = textoCardapio(p.gradeCardapio);
    p.gradeCardapio = serializarCardapio(p.gradeCardapio);
    if (p.texto.length > 30000)
      throw new Error("Reduza as observações do cardápio para publicar.");
  }
